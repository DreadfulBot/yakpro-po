    ...
    try {
        ...
        if ($conf->shuffle_stmts && (count($stmts) > 2)) {
            ...
            foreach ($stmts as $i => $stmt) {                      // if a use statement exists, do not shuffle before the last use statement ... same as for declare statement for strict_types and encoding
                ...
                if ($stmt instanceof PhpParser\Node\Stmt\Use_ || $stmt instanceof PhpParser\Node\Stmt\GroupUse) {
                    $last_use_stmt_pos = $i;
                }
                if ($stmt instanceof PhpParser\Node\Stmt\Declare_ && isset($stmt->declares[0]->key->name)) {
                    ...
                    if (($str_tmp == "strict_types") || ($str_tmp == "encoding")) {
                        $last_use_stmt_pos = $i;
                    }
                }
            }
        ...
        $code  = '<?php'.PHP_EOL;
        if (isset($conf->comment_intro) && $conf->comment_intro) {
    ...
    if ($dp = opendir($path)) {
        while (($entry = readdir($dp)) !==  false) {
            ...
            if (is_link("$path/$entry")) {
                unlink("$path/$entry");
            }            // remove symbolic links first, to not dereference...
...
function obfuscate_directory($source_dir, $target_dir, $keep_mode = false)   // self-explanatory recursive obfuscation
{
    ...
    while (($entry = readdir($dp)) !== false) {
        ...
        $source_stat = @lstat($source_path); //echo "source [$source_path]\n";
...
/**
 * Alternative to php_strip_whitespace() that preserves all comments
 *
 * @param string $code PHP code to process
 * @return string Minified code with preserved comments
 */
function strip_whitespace_keep_comments($code)
{
...
function remove_whitespaces($str)
{
    ...
    if ($conf->preserve_doc_comments) {
        // save file with replaced /n and /r
        $str = strip_whitespace_keep_comments($str);
    } else {
